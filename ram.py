...
from tensorflow.python.keras.layers import Input, Dense, Activation, LSTM, Embedding, Bidirectional, Lambda
...
class RecurrentAttentionMemory:
    def __init__(self):
        ...
        if os.path.exists('ram_saved_model.h5'):
            ...
            memory = Bidirectional(LSTM(**self.LSTM_PARAMS, return_sequences=True), name='memory')(sentence)
            aspect = Bidirectional(LSTM(**self.LSTM_PARAMS, return_sequences=True), name='aspect')(aspect)
